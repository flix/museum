...
`2.1.2`, while entrance and gift shop remain at `2.0.1` and each still requires
clerk `2.1.1`. All three consumers therefore use clerk `2.1.2` in this build.
...
artifact contents. It records the clerk `2.1.1` manifest read during resolution,
the version entrance and gift shop require, with a `toml` digest only, alongside
clerk `2.1.2` with both `toml` and `fpkg` digests because `2.1.2` is the archive
actually selected.
